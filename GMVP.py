...
def GMVP(H):
    ...
    ones_vec = np.ones(H.shape[0])
    nom = np.dot(inv_H, ones_vec)
    w = nom / np.dot(ones_vec.T, nom)
    return w
...
if __name__ == '__main__':
    universe = 'DJIA'
    horizon = 1
    ret_df = load_ret(universe, horizon)
    ...
    all_sum_df, all_std_df = Result(ret_df, vech_df, 'Forecast_Cov', 'GHAR', universe, horizon, port='')
    all_sum_df = all_sum_df.loc[rank_files_l]
    print(all_sum_df)
    all_sum_df.to_csv(join(path, 'Results_Analysis', f'{universe}_Cov_GMVP_F{horizon}_Daily.csv'))