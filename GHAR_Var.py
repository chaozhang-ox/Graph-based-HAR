...
parser.add_argument("--adj_name", type=str, default='iden+global', help="model name")
...
def load_adjacency_sector(universe):
    adj_df = pd.read_csv(join(path, 'Data', f'{universe}_sector_adj.csv'), index_col=0)
...
def Train(model_name, ret_df, vech_df, subdf_dic, date, date_l):
    ...
    gics_adj_df = load_adjacency_sector(opt.universe)
    n = vech_df.shape[1]
    adj_name_l = opt.adj_name.split('+')
    adj_df_l = []
    for adj_name in adj_name_l:
        if adj_name == 'iden':
            adj_df = pd.DataFrame(np.identity(n), index=gics_adj_df.index, columns=gics_adj_df.columns)
        elif adj_name == 'global':
            adj_df = pd.DataFrame(np.ones((n, n)), index=gics_adj_df.index, columns=gics_adj_df.columns)
            adj_df -= np.identity(n)
            adj_df /= adj_df.sum()
        elif adj_name == 'sector':
            adj_df = gics_adj_df - np.identity(n)
            adj_df /= (adj_df.sum() + 1e-8)
        elif adj_name == 'glasso':
            adj_df = GLASSO_Precision(subret)
        else:
            adj_df = pd.DataFrame(np.zeros((n, n)), index=gics_adj_df.index, columns=gics_adj_df.columns)
